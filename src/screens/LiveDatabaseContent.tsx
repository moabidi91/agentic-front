...
//
// Extracted from LiveDatabaseScreen so it can also be embedded as a
// Settings-modal tab (see SettingsModal.tsx) without duplicating logic.
export function LiveDatabaseContent({ embedded = false }: { embedded?: boolean }) {
  const api = useApi();
  ...
  return (
    <div
      style={{
        ...
        padding: embedded ? '4px 2px 2px' : '20px 28px',
        gap: 16,
        overflowY: 'auto',
      }}
    >
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {!embedded && <span style={{ fontSize: 15, fontWeight: 700 }}>Live database</span>}
          <Badge bg="var(--green-soft)" color="var(--green)" dot>
            live
          </Badge>
        </div>
        <Button variant="danger" size={embedded ? 'sm' : 'md'} onClick={() => setConfirmClear(true)}>
          Clear database
        </Button>
      </div>

      {cleared && <div style={{ fontSize: 11.5, color: 'var(--text-3)' }}>Database cleared. Rows below reflect the empty state.</div>}
      ...
      <div className="af-card" style={{ overflow: 'auto' }}>
      ...
      <div style={{ fontSize: 10.5, color: 'var(--text-3)', maxWidth: 560 }}>
        Read-only — writes only ever come from the backend. Clear database is a dev/demo action: it wipes every session, including
        any still running.
      </div>
